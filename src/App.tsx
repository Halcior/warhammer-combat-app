...
const expectedResult = useMemo(() => {
  ...
  <div className="top-grid">
    <div className="card">
      ...
      setConditions((prev) => ({...prev, targetModelCount: value,
    }));
  }}
/>
      </label>
...
<div className="rules-list">
  {activeModifierRules.length > 0 ? (
    activeModifierRules.map((rule, index) => (
...
<div className="stat-box">
  ...
  <span className="stat-value">{expectedResult.criticalHits?.toFixed(2) ?? "0.00"}</span>
</div>

<div className="stat-box">
  <span className="stat-label">Extra hits</span>
  <span className="stat-value">{expectedResult.extraHitsFromSustained?.toFixed(2) ?? "0.00"}</span>
</div>
<div className="stat-box">
